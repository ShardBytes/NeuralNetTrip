...
class TableAgent(agent.Agent):
    def __init__(self, environment, gamma = 0.9, epsilon_training = 0.2, epsilon_testing = 0, learning_factor = 0.15):
    ...
    def main(self):
        ...
        #učenie
        maxQValue = self.qTable[self.state].max() #tiež numpy metóda!

        #learning factor
        remembered_stuff = (1.0 - self.learning_factor) * self.qTable[self.previous_state][self.previous_action]
        new_stuff = self.learning_factor * (reward + self.gamma * maxQValue)
